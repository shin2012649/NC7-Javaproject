-- 회원
DROP TABLE IF EXISTS javaproject_user RESTRICT;

-- 게시글
DROP TABLE IF EXISTS javaproject_board RESTRICT;

-- 게시판유형
DROP TABLE IF EXISTS javaproject_board_category RESTRICT;

-- 게시글첨부파일
DROP TABLE IF EXISTS javaproject_board_file RESTRICT;

-- 회원
CREATE TABLE javaproject_user (
  user_no    INTEGER      NOT NULL COMMENT '번호', -- 번호
...
-- 회원
ALTER TABLE javaproject_user
  ADD CONSTRAINT PK_javaproject_user -- 회원 기본키
  PRIMARY KEY (
  user_no -- 번호
  );

-- 회원 유니크 인덱스
CREATE UNIQUE INDEX UIX_javaproject_user
  ON javaproject_user ( -- 회원
    email ASC -- 이메일
  );

ALTER TABLE javaproject_user
  MODIFY COLUMN user_no INTEGER NOT NULL AUTO_INCREMENT COMMENT '번호';
...
ALTER TABLE javaproject_board
  ADD CONSTRAINT PK_javaproject_board -- 게시글 기본키
  PRIMARY KEY (
  board_no -- 번호
  );
...
-- 게시글
ALTER TABLE javaproject_board
  ADD CONSTRAINT FK_javaproject_user_TO_javaproject_board -- 회원 -> 게시글
  FOREIGN KEY (
  writer -- 작성자
  )
  REFERENCES javaproject_user ( -- 회원
  user_no -- 번호
  );
