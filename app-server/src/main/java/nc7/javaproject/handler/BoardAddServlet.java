    ...
    try {
      ...
      for (Part part : request.getParts()) {
        if (part.getName().equals("files") && part.getSize() > 0) {
          String uploadFileUrl = nc7.javaproject.handler.InitServlet.ncpObjectStorageService.uploadFile(
      ...
      nc7.javaproject.handler.InitServlet.boardDao.insert(board);
      if (attachedFiles.size() > 0) {
        nc7.javaproject.handler.InitServlet.boardDao.insertFiles(board);
      }
